...
namespace WinformProtocol
{
                ...
                try
                {
                    if (db.open_connection())
                    {
                        ...
                        using (NpgsqlCommand cmd = db._conn.CreateCommand())
                        {
                            ...
                            List<string> _displayListForQuery = new List<string>();
                            ...
                            var _displayForQuery = tableEnumerable.ToArray();
                            ///
                            foreach (DataRow row in tbcode.Rows)
                            {
                                string clnnamevalue = Convert.ToString(row["clnnamevalue"]);
                                _paramListForQuery.Add(clnnamevalue);
                            }
                            _paramListForQuery.ToArray();

                            foreach (DataRow row2 in tbcode2.Rows)
                            {
                                string displayListForQuery = Convert.ToString(row2["display_name"]);
                                _displayListForQuery.Add(displayListForQuery);
                            }
                            _displayListForQuery.ToArray();

                            DataTable dt_source = null;
                            dt_source = db5m.get_all_custom(dtpDateFrom, dtpDateTo, _paramListForQuery);

                            //Voi moi row - gia tri dc dump tu trong db
                            foreach (DataRow row3 in dt_source.Rows)
                            {
                                ...
                                for (int i = 0; i < _paramListForQuery.Count; i++)
                                {
                                    ...
                                    string code = _paramListForQuery[i];
                                    for (int j = 0; j < _displayForQuery.Count(); j++)
                                    {
                                        DataRow currentRow = _displayForQuery[j];
                                        if (currentRow["item_name"].Equals(code))
                                        {
                                            string display_name = currentRow["display_name"].ToString();
    ...
    public class Control
    {
        ...
        public void Protocol(SerialPort serialPortTN, SerialPort serialPortTP, SerialPort serialPortTOC, SerialPort serialPortSAMP, TcpListener tcpListener, measured_data objMeasuredDataGlobal)
        {
            try
            {
                String data = null;
        ...
        public void doChat(TcpClient client, String data, ASCIIEncoding _encoder, SerialPort serialPortTN, SerialPort serialPortTP, SerialPort serialPortTOC, SerialPort serialPortSAMP, measured_data objMeasuredDataGlobal)
        {
            int requestCount = 0;
                                ...
                                try
                                {
                                    ...
                                    if (i != 0)
                                    {
                                        ...
                                        if (true)
                                        {
                                            ...
                                            passingParamInit(serialPortTN, serialPortTP, serialPortTOC, serialPortSAMP, nwStream, buffer, data, client, username, objMeasuredDataGlobal);
                                            break;
                                        }
                                        else
                                        {
        ...
        public void passingParamInit(Form1 form, SerialPort serialPortTN, SerialPort serialPortTP, SerialPort serialPortTOC, SerialPort serialPortSAMP, NetworkStream nwStream, byte[] buffer, String data, TcpClient client, String username, measured_data objMeasuredDataGlobal)
        {
        ...
        public void passingParamInit(SerialPort serialPortTN, SerialPort serialPortTP, SerialPort serialPortTOC, SerialPort serialPortSAMP, NetworkStream nwStream, byte[] buffer, String data, TcpClient client, String username, measured_data objMeasuredDataGlobal)
        {
                                    ...
                                    try
                                    {
                                        ...
                                        if (BytesRead != 0)
                                        {
                                            ...
                                            if (true)
                                            {
                                                ...
                                                control.init(newstationid, null, Control.serialPortTN, Control.serialPortTP, Control.serialPortTOC, Control.serialPortSAMP, nwStream, buffer, data, ClientSocket, username, Control.objMeasuredDataGlobal);
                                                break;
                                            }
                                            else
                                            {
