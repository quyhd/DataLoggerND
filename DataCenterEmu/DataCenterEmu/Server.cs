...
namespace Protocol
{
    public static class MyTcpListener
    {
        ...
        //    String mps_ph = ConvertStr(DataLogger.frmNewMain.objMeasuredDataGlobal.var1.ToString(), 10);
        //    String mps_ph_status = ConvertStr(DataLogger.frmNewMain.objMeasuredDataGlobal.var1_status.ToString(), 2);
        ...
        public static void RDAT(byte[] buffer, String data, int j, NetworkStream nwStream, TcpClient client,measured_data objMeasuredDataGlobal)
        {
           ...
           // String item;
            ...
            (new byte[] { 0x03 }).CopyTo(_ETX, 0);
            #region item
            String var1_value = new user_repository().ConvertStr(objMeasuredDataGlobal.var1.ToString(), 10);
            String var1_status = new user_repository().ConvertStr(objMeasuredDataGlobal.var1_status.ToString(), 2);
            byte[] _var1_code = new byte[5];
            byte[] _var1_value = new byte[10];
            byte[] _var1_status = new byte[2];
            _encoder.GetBytes("ndvar1").CopyTo(_var1_code,0);
            _encoder.GetBytes(var1_value).CopyTo(_var1_value, 0);
            _encoder.GetBytes(var1_status).CopyTo(_var1_status, 0);
            byte[] _var1 = _var1_code.Concat(_var1_value).Concat(_var1_status).ToArray();

            String var2_value = new user_repository().ConvertStr(objMeasuredDataGlobal.var2.ToString(), 10);
            String var2_status = new user_repository().ConvertStr(objMeasuredDataGlobal.var2_status.ToString(), 2);
            byte[] _var2_code = new byte[5];
            byte[] _var2_value = new byte[10];
            byte[] _var2_status = new byte[2];
            _encoder.GetBytes("ndvar2").CopyTo(_var2_code,0);
            _encoder.GetBytes(var2_value).CopyTo(_var2_value,0);
            _encoder.GetBytes(var2_status).CopyTo(_var2_status, 0);
            byte[] _var2 = _var2_code.Concat(_var2_value).Concat(_var2_status).ToArray();

            String var3_value = new user_repository().ConvertStr(objMeasuredDataGlobal.var3.ToString(), 10);
            String var3_status = new user_repository().ConvertStr(objMeasuredDataGlobal.var3_status.ToString(), 2);
            byte[] _var3_code = new byte[5];
            byte[] _var3_value = new byte[10];
            byte[] _var3_status = new byte[2];
            _encoder.GetBytes("ndvar3").CopyTo(_var3_code, 0);
            _encoder.GetBytes(var3_value).CopyTo(_var3_value, 0);
            _encoder.GetBytes(var3_status).CopyTo(_var3_status, 0);
            byte[] _var3 = _var3_code.Concat(_var3_value).Concat(_var3_status).ToArray();

            String var4_value = new user_repository().ConvertStr(objMeasuredDataGlobal.var4.ToString(), 10);
            String var4_status = new user_repository().ConvertStr(objMeasuredDataGlobal.var4_status.ToString(), 2);
            byte[] _var4_code = new byte[5];
            byte[] _var4_value = new byte[10];
            byte[] _var4_status = new byte[2];
            _encoder.GetBytes("ndvar4").CopyTo(_var4_code, 0);
            _encoder.GetBytes(var4_value).CopyTo(_var4_value, 0);
            _encoder.GetBytes(var4_status).CopyTo(_var4_status, 0);
            byte[] _var4 = _var4_code.Concat(_var4_value).Concat(_var4_status).ToArray();

            String var5_value = new user_repository().ConvertStr(objMeasuredDataGlobal.var5.ToString(), 10);
            String var5_status = new user_repository().ConvertStr(objMeasuredDataGlobal.var5_status.ToString(), 2);
            byte[] _var5_code = new byte[5];
            byte[] _var5_value = new byte[10];
            byte[] _var5_status = new byte[2];
            _encoder.GetBytes("ndvar5").CopyTo(_var5_code, 0);
            _encoder.GetBytes(var5_value).CopyTo(_var5_value, 0);
            _encoder.GetBytes(var5_status).CopyTo(_var5_status, 0);
            byte[] _var5 = _var5_code.Concat(_var5_value).Concat(_var5_status).ToArray();

            String var6_value = new user_repository().ConvertStr(objMeasuredDataGlobal.var6.ToString(), 10);
            String var6_status = new user_repository().ConvertStr(objMeasuredDataGlobal.var6_status.ToString(), 10);
            byte[] _var6_code = new byte[5];
            byte[] _var6_value = new byte[10];
            byte[] _var6_status = new byte[2];
            _encoder.GetBytes("ndvar6").CopyTo(_var6_code, 0);
            _encoder.GetBytes(var6_value).CopyTo(_var6_value, 0);
            _encoder.GetBytes(var6_status).CopyTo(_var6_status, 0);
            byte[] _var6 = _var6_code.Concat(_var6_value).Concat(_var6_status).ToArray();
            ...
            byte[] _item = _var1.Concat(_var2).Concat(_var3).Concat(_var4).Concat(_var5).Concat(_var6).ToArray();
            ...
            _item = _encoder.GetBytes("06").Concat(_item).ToArray();
            String checksum = CalculateChecksum(_encoder.GetString(_header) + _encoder.GetString(_item) + _encoder.GetString(_ETX));
            byte[] _checksum = new byte[2];
            _encoder.GetBytes(checksum).CopyTo(_checksum,0);
            byte[] _CR = new byte[1];
        ...
        public static void RCHK(byte[] buffer, String data, int j, NetworkStream nwStream, TcpClient client) {
        ...
        public static void SAMP(byte[] buffer, String data, int j, NetworkStream nwStream, TcpClient client) {
        ...
        public static void init(NetworkStream nwStream, byte[] buffer, String data, TcpClient client, String username, measured_data objMeasuredDataGlobal)
        {
            int i;
            try
            {
                            ...
                            if (_encoder.GetString(buffer.SubArray(j + 15, 4)).Equals("RDAT"))
                            {
                                ...
                                if (_encoder.GetString(buffer.SubArray(j + 15, 4)).Equals("DUMP"))
                                {
                                    ...
                                    if (_encoder.GetString(buffer.SubArray(j + 15, 4)).Equals("CCHK"))
                                    {
                                        ...
                                        {
                                            RCHK(buffer, data, j, nwStream, client);
                                            break;
                                        }
                                        else
                                            if (_encoder.GetString(buffer.SubArray(j + 15, 4)).Equals("SAMP"))
                                            {
                                                SAMP(buffer,data,j,nwStream,client);
                                                /*
        ...
        public static void Protocol(TcpListener server, measured_data objMeasuredDataGlobal)
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    newThread = new Thread(() => doChat(client, data, _encoder,objMeasuredDataGlobal));
                    newThread.Start();
                    Thread.Sleep(100);
                }
                client.Close();
        ...
        public static void doChat(TcpClient client, String data, ASCIIEncoding _encoder, measured_data objMeasuredDataGlobal)
        {
            int requestCount = 0;
            ...
            string _publicKey;
            requestCount = 0;
            station stationSettings = new station_repository().get_info();
            ...
            while (true)
            {
                try
                {
                    ...
                    if (i != 0)
                    {
                        ...
                        String stationid = stationSettings.station_id;
                        string[] separators = { "\\" };
                        try
                        {
                            ...
                            if ((buffer[0] == 5 && datas[2].Remove(datas[2].Length - 1, 1).Equals(stationid)) || (buffer[0] == 5 && datas[2].Remove(datas[2].Length - 2, 2).Equals(stationid)) || (buffer[0] == 5 && datas[2].Equals(stationid)))
                            {
                                ...
                                {
                                    Console.WriteLine("SENDING: " + "\"" + _encoder.GetString(sender) + "\"" + " BUT");
                                    Console.WriteLine("ERROR : CAN NOT LISTEN ANY CONNECT, CHECK CONNECT IN CENTER.");
                                }
                                ...
                                try
                                {
                                    ...
                                    if (i != 0 )
                                    {
                                        ...
                                        if (true)
                                        {
                                            ...
                                            MyTcpListener.init(nwStream, buffer, data, client, username, objMeasuredDataGlobal);
                                        }
                                        else
                                        {
                                    ...
                                    {
                                        MyTcpListener.sendByte(nwStream, _encoder.GetBytes("ERROR 1"));
                                    }
                                }
                                catch
                                {
